...
<form class="form row" method="POST" action="" name="myForm" onSubmit="validate()">
    ...
    if (isset($_GET['id'])) {
        $id= $_GET['id'];
        $squeril =mysqli_query($mysqli,"SELECT * FROM kegiatan WHERE id='$id'");
        while ($row1= mysqli_fetch_array($squeril)) {
            $isi =$row1['isi'];
...
<script>
    function validate() {
        if (document.myForm.isi.value == "") {
            ...
            alert("silahkan lengkapi tangal awal");
            document.myForm.isi.focus();
            return false;
            
        } else if (document.myForm.tgl_akhr.value =="") {
            alert("silahkan lengkapi tangal akhir");
            document.myForm.isi.focus();
            return false;
        } else{
            return true;
